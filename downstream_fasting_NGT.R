...
save.image(file ="/home/admin/R_data/GSE153837_normalized_22-10-2023/split-data/fasting_NGT/results/downstream_fasting_NGT/downstream_fasting_NGT.RData")

# 
# # Loading all workspace env data objects from GSE-datpreprocessing-8-10-23.RData
load(file ="/home/admin/R_data/GSE153837_normalized_22-10-2023/split-data/fasting_NGT/results/downstream_fasting_NGT/downstream_fasting_NGT.RData")
# 
...
datTraits_selective <- datTraits[,-c(1,3,4,7,8,9,10,11)]


MEs0=moduleEigengenes(datExpr,moduleColors)$eigengenes
MEs=orderMEs(MEs0)
moduleTraitCor=cor(MEs,datTraits_selective,use='p')
moduleTraitPvalue=corPvalueStudent(moduleTraitCor,nSamples)

heatmap.data <- merge(MEs, datTraits_selective, by = 'row.names')
head(heatmap.data)
...
# SVG graphics device  https://r-coder.com/save-plot-r/
svg("/home/admin/R_data/GSE153837_normalized_22-10-2023/split-data/fasting_NGT/results/downstream_fasting_NGT/module_trait_relationship_fasting_NGT.svg")
...
CorLevelPlot(heatmap.data,
             ...
             col = c("#D7191C", "#FDAE61", "#FFFFBF", "#ABD9E9", "#2C7BB6"))  #brewer.pal(n = 5, name = "RdYlBu")
                                                                              #"#D7191C" "#FDAE61" "#FFFFBF" "#ABD9E9" "#2C7BB6"
                                                                              #("blue1", "skyblue", "white", "pink", "red")
                                                                              #"#E66101" "#FDB863" "#F7F7F7" "#B2ABD2" "#5E3C99" purple orange
...
# 
# write.csv(geneInfo,file="geneInfo_fasting_NGT.csv")
# 
# 
# #----Gene module membership and trait significance and save GeneInfo CSV file for FASTING INSULIN-----
# # Define a variable containing the trait column of datTrait
# fasting_insulin = as.data.frame(datTraits$Fast_Insulin);
# names(fasting_insulin) = "Fasting Insulin"
# 
# # names (colors) of the modules
# modNames = substring(names(MEs), 3)
# 
...
# names(MMPvalue)=paste("p.MM",modNames,sep="")
# 
# geneTraitSignificance=as.data.frame(cor(datExpr,fasting_insulin,use="p"))
# GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
# names(geneTraitSignificance) = paste("GS.", names(fasting_insulin), sep="");
# names(GSPvalue) = paste("p.GS.", names(fasting_insulin), sep="")
# 
# # dataframe with Most significant genes associated with Trait of interest
# #Gyaan: # # Using the gene significance you can identify genes that have a high significance for trait of interest 
# # # Using the module membership measures you can identify genes with high module membership in interesting modules.
...
# gene_trait_significance_pval <- as.data.frame(GSPvalue)#here trait of interest was fasting glucose
# gene_trait_significance_pval$ILMN_ID <- rownames(gene_trait_significance_pval)
# sorted_gene_trait_significance_pval <- gene_trait_significance_pval[order(gene_trait_significance_pval$`p.GS.Fasting Insulin`),]
# x=match(sorted_gene_trait_significance_pval$ILMN_ID,annot$ILMN_ID)
# y=data.frame(ILMN_ID=sorted_gene_trait_significance_pval$ILMN_ID,
#              GSP_val=sorted_gene_trait_significance_pval$`p.GS.Fasting Insulin`,
#              gene_symbol=annot$SYMBOL[x],
#              definition=annot$DEFINITION[x])
# write.csv(y,file="genes_significantly_associated_with_fasting_insulin.csv")
# 
# # sanity check
# names(datExpr)
# names(datExpr)[moduleColors=="purple"]
...
# 
# #ordering module by their significance for choice of trait
# modOrder=order(-abs(cor(MEs,fasting_insulin,use='p')))
# 
# #adding module membership information in the chosen order in previous line
# for (mod in 1:ncol(geneModuleMembership))
# {
...
# #rearranging the data and saving a csv file
# 
# geneOrder=order(geneInfo0$moduleColor,-abs(geneInfo0$GS.Fasting.Insulin))
# geneInfo=geneInfo0[geneOrder,]
# 
# write.csv(geneInfo,file="geneInfo_fasting_insulin_NGT.csv")
# 
# 
# 
# 
# #----Gene module membership and trait significance and save GeneInfo CSV file for HOMA IR-----
# # Define a variable containing the trait column of datTrait
# homa_ir = as.data.frame(datTraits$HOMA_IR);
# names(homa_ir) = "HOMA IR"
# 
# # names (colors) of the modules
# modNames = substring(names(MEs), 3)
# 
...
# names(MMPvalue)=paste("p.MM",modNames,sep="")
# 
# geneTraitSignificance=as.data.frame(cor(datExpr,homa_ir,use="p"))
# GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
# names(geneTraitSignificance) = paste("GS.", names(homa_ir), sep="");
# names(GSPvalue) = paste("p.GS.", names(homa_ir), sep="")
# 
# # dataframe with Most significant genes associated with Trait of interest
# #Gyaan: # # Using the gene significance you can identify genes that have a high significance for trait of interest 
# # # Using the module membership measures you can identify genes with high module membership in interesting modules.
...
# #annot=read.csv(file = "/home/admin/R_data/GSE153837_normalized_22-10-2023/Anno.csv")#annotation Data
# 
# gene_trait_significance_pval <- as.data.frame(GSPvalue)#here trait of interest was homa IR
# gene_trait_significance_pval$ILMN_ID <- rownames(gene_trait_significance_pval)
# sorted_gene_trait_significance_pval <- gene_trait_significance_pval[order(gene_trait_significance_pval$`p.GS.HOMA IR`),]
# x=match(sorted_gene_trait_significance_pval$ILMN_ID,annot$ILMN_ID)
# y=data.frame(ILMN_ID=sorted_gene_trait_significance_pval$ILMN_ID,
#              GSP_val=sorted_gene_trait_significance_pval$`p.GS.HOMA IR`,
#              gene_symbol=annot$SYMBOL[x],
#              definition=annot$DEFINITION[x])
# write.csv(y,file="genes_significantly_associated_with_homa_ir.csv")
# 
# # sanity check
# names(datExpr)
# names(datExpr)[moduleColors=="purple"]
...
# 
# #ordering module by their significance for choice of trait
# modOrder=order(-abs(cor(MEs,homa_ir,use='p')))
# 
# #adding module membership information in the chosen order in previous line
# for (mod in 1:ncol(geneModuleMembership))
# {
...
# #rearranging the data and saving a csv file
# 
# geneOrder=order(geneInfo0$moduleColor,-abs(geneInfo0$GS.HOMA.IR))
# geneInfo=geneInfo0[geneOrder,]
# 
# write.csv(geneInfo,file="geneInfo_HOMA_IR_NGT.csv")
# 
# 
# #----Plot for Gene Module Membership and Gene Trait Significance for Trait of Interest----
# 
# module = "blue" #change as required
...
# 
# # SVG graphics device  https://r-coder.com/save-plot-r/
# svg("/home/admin/R_data/GSE153837_normalized_22-10-2023/split-data/fasting_NGT/results/downstream_fasting_NGT/GS_MM_fasting_glucose_blue_module.svg")
# 
# # Define the x and y ticks
# x_ticks <- seq(0, 1, by = 0.2)  # Adjust the range and interval as needed
# y_ticks <- seq(0, 1, by = 0.2)  # Adjust the range and interval as needed
# 
...
# 
# # Add custom x-axis
# axis(1, at = x_ticks, labels = x_ticks, cex.axis = 1.2)
# 
# # Add custom y-axis
# axis(2, at = y_ticks, labels = y_ticks, cex.axis = 1.2)
# 
# 
# 
# dev.off()
...
#choosingtophubgene in each module function---------------

colorh=labels2colors(net$colors)
hubs_genes_in_modules <- as.data.frame(chooseTopHubInEachModule(omitColors = 'FALSE',datExpr,colorh))
colnames(hubs_genes_in_modules)[1] <- 'ILMN_ID'
hubs_genes_in_modules$modulecolor <- row.names(hubs_genes_in_modules)
# Merge the data frames based on the common column ILMN_ID
hub_gene_merged_df <- merge(hubs_genes_in_modules,annot, by = "ILMN_ID")

write_csv(hub_gene_merged_df,file='hub_gene_merged_df.csv')
# #----------------------------------E X T R A-------------------------------------
